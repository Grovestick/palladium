...
class Tray:
    def __init__(self, attach=False, inside=False):
        self.proc = None
        self.icon = None
        self.status = "starting"
        # inside the server: there is no other process to start, stop or restart
        self.inside = inside
        if not attach and not inside:
            self.start_server()
    ...
    def menu(self):
        return pystray.Menu(
            ...
            pystray.Menu.SEPARATOR,
            *([] if self.inside else [pystray.MenuItem("Restart server", self.restart)]),
    ...
    def quit(self):
        if self.inside:
            # the icon is the server's own: Quit ends the server with it
            if self.icon:
                self.icon.stop()
            os._exit(0)
        self.stop_server()
    ...
    def run(self):
        ...
        # down - the icon has to be taken off the tray by the program that put it
        # there. Nobody else can, so a program that simply stops leaves a dead P
        # sitting in the tray until somebody happens to hover over it.
        def bow_out(*_):
            try:
                self.quit()
            except Exception:
        ...
        for name in ("SIGTERM", "SIGBREAK", "SIGINT"):
            ...
            if sig is not None:
                try:
                    ...
                    pass          # not the main thread, or not this platform
        atexit.register(bow_out)
        self.icon = pystray.Icon("palladium", mark(64), "Palladium", self.menu())
        self.icon.run()


def start_inside():
    """The icon, run by the server itself on a thread of its own. Nothing if another
    icon is already up."""
    if sys.platform != "win32" or not only_one():
        return None
    tray = Tray(inside=True)
